...
#---------------------------------------------------------------------------------------------------------
    def button_click(hw, caller, clicked):
        ...
        if clicked == "login":
            ...
            hw.links = hw.fun.scrape_job_list()
            print("000000000000000")

            hw.job_title_40 = "Job Lists"
            parts_job_company = "Links"
            hw.job_company_first2part = str(len(hw.links)) + " links being collected"

            hw.can_small.itemconfig(hw.head1, text = hw.job_title_40)
            hw.can_small.itemconfig(hw.head2, text = hw.job_company_first2part)
            hw.ui.def_text_bottom(hw,"<<< click Save to Excel to save them.")
            ...
            df = pd.DataFrame(hw.links)
            writer = pd.ExcelWriter('LinkedinJobList.xlsx', engine='xlsxwriter')
